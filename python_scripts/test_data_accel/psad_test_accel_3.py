...
if len(sys.argv) < 2:
   ...
   ser.write(struct.pack('BB', 0x0c, 0x03))
   wait_for_ack()
...
# read incoming data
   ddata = ""
   numbytes = 0
   framesize = 23 # 6byte emg, 12byte accel, 6byte gyro, 8byte strain gauge, 3 byte status+ts
   found0 = 0       
   ...
   try:
      while True:
         ...
         #(accel1xl, accel1xh, accel2xl, accel2xh) = struct.unpack('BBBB', data[9:13])
         ...
         (gyroxl, gyroxh, gyroyl, gyroyh, gyrozl, gyrozh) = struct.unpack('BBBBBB', data[9:15])
         ...
         if gyroz > 0x7FFF:
             gyroz -= 0x10000         
         print "0x%02x,%02x,%02x,%1x|%5d,%5d,%5d" % (data0,ts0, ts1, sw1,gyrox, gyroy, gyroz)
